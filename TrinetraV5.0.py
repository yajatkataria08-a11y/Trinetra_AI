...
class TrinetraEngine:
    ...
    def get_all_vectors(self):
        try:
            return faiss.vector_to_array(self.index.reconstruct_n(0, self.index.ntotal))
        except Exception:
            return None
        ...
        with cols[idx % 3]:
            ...
            with st.expander("Feedback"):
                ...
                if st.button("Submit Rating", key=f"submit_rate_{r['id']}_{idx}"):
                    image_engine.metadata_db.add_rating(r['id'], 
                                                       st.session_state.user['username'], 
                                                       new_rating)
                ...
                if st.button("Post Comment", key=f"post_{r['id']}_{idx}") and new_comment:
                    image_engine.metadata_db.add_comment(r['id'], 
                                                        st.session_state.user['username'],
                                                        new_comment)
    ...
    if mode == "Text Query":
        ...
        if q and st.session_state.show_suggestions:
            ...
            if suggestions:
                ...
                for sug in suggestions:
                    sug_html += f'<span class="suggestion-chip">{sug}</span>'
        ...
        if st.button("Run Visual Scan", key="vs_txt") and q:
            ...
            st.session_state.search_history.append({
                "query": q, "modality": "image", "timestamp": time.time(), 
...
with tab_a:
    ...
    if st.button("Run Acoustic Scan") and q:
        ...
        st.session_state.search_history.append({
            "query": q, "modality": "audio", "timestamp": time.time(),
            "results_count": len(results)
        })
        st.caption(f"Search time: {ms:.0f} ms")
