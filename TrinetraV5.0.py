...
class EmailOTPSender:
    ...
    def is_configured(self):
        ...
        return (
            ...
            and "@" in self.sender_email
            and bool(self.sender_password)
    ...
    def test_connection(self):
        ...
        if not self.is_configured():
            return False, (
                "SMTP not configured. Add SMTP_EMAIL and SMTP_PASSWORD to "
        ...
        try:
            ...
            return True, f"✅ SMTP connected successfully as {self.sender_email}"
        except smtplib.SMTPAuthenticationError:
            return False, (
                "❌ Gmail authentication failed. You must use a Gmail App Password — "
                "NOT your normal Gmail password.\n"
                "Go to: Google Account → Security → 2-Step Verification → App Passwords → "
                "Generate one for 'Mail' + 'Other (Trinetra)'."
            )
        except smtplib.SMTPConnectError:
            return False, "❌ Cannot connect to smtp.gmail.com:587. Check network/firewall."
    ...
    def send_otp_email(self, recipient_email, otp, username):
        ...
        try:
            ...
            msg = "Gmail auth failed — use an App Password, not your normal Gmail password."
            logger.error(f"SMTP auth error sending to {recipient_email}: {msg}")
            return False, msg, otp
        except smtplib.SMTPRecipientsRefused:
            msg = f"Recipient {recipient_email} was refused by Gmail."
            logger.error(f"SMTP recipient refused: {msg}")
            return False, msg, otp
        except Exception as e:
